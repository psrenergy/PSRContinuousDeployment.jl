...
RUN echo "#!/bin/bash" > entrypoint.sh && \
    ...
    echo "git clone -n https://github.com/psrenergy/\$GITHUB_REPOSITORY.git" >> entrypoint.sh && \
    echo "cd \$GITHUB_REPOSITORY" >> entrypoint.sh && \
    echo "git checkout \$GITHUB_REFERENCE" >> entrypoint.sh && \
