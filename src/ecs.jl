...
function start_ecs_task(;
    ...
    @info("ECS: Task definition: $task_definition")
...
function stop_ecs_task(task_id::AbstractString, retries::Integer = 20, delay::Integer = 15)
    try
        @info("ECS: Stopping task $task_id...")
        ...
        for _ in 1:retries
            ...
            if task_status in ["STOPPED", "DEACTIVATING", "DEPROVISIONING"]
                @info("ECS: Task $task_id stopped successfully")
                return true
            end
            sleep(delay)
        end
    catch e
        @warn("ECS: Failed to stop task $task_id: $e")
    end

    @info("ECS: Task $task_id did not stop within $retries retries")
...
function start_ecs_task_and_watch(;
    ...
    @info("ECS: https://us-east-1.console.aws.amazon.com/ecs/v2/clusters/automations/tasks/$task_id/logs?region=us-east-1")
    @info("ECS: https://us-east-1.console.aws.amazon.com/cloudwatch/home?region=us-east-1#logsV2:log-groups/log-group/%2Fecs%2F$task_definition/log-events/ecs%2F$task_definition%$task_id")
    ...
    try
        while true
            ...
            if status != last_status
                @info("ECS: Task $task_id status: $status")
                last_status = status
            end

            if status == "STOPPED"
                @info("ECS: Task $task_id finished")
                break
        ...
        if e isa InterruptException
            @warn("ECS: Task $task_id interrupted")
        else
            @error("ECS: An error occurred: $e\n$(catch_backtrace())")
        end
    finally
    ...
    @info("ECS: Task $task_id exit code: $exit_code")
